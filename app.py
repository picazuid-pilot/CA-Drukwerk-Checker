...
class CAFlyerChecker:
    ...
    def _load_reference_logos(self):
        ...
        if self.assets_folder.exists():
            for file in self.assets_folder.glob("*.png"):
                # Skip engelstalige logo's (herkend aan _en of -en suffix, niet losse 'en' substring)
                if not self._is_english_filename(file.stem):
                    try:
    ...
    def _load_english_logos(self):
        ...
        if self.assets_folder.exists():
            for file in self.assets_folder.glob("*.png"):
                if self._is_english_filename(file.stem):
                    try:
                        img = Image.open(file).convert('RGBA')
    ...
    def _is_english_filename(self, stem):
        """
        Herken engelstalige logo's aan een duidelijke '_en' of '-en' suffix/prefix,
        of het losse woord 'english'/'eng'. Voorkomt valse hits op bestandsnamen
        die toevallig de letters 'en' bevatten (bv. 'logo_negen.png').
        """
        stem_lower = stem.lower()
        parts = stem_lower.replace("-", "_").split("_")
        return (
            "en" in parts
            or "eng" in parts
            or "english" in parts
            or stem_lower.endswith("_en")
            or stem_lower.endswith("-en")
        )
    ...
    def check_distortion(self, img_array, reference_array, aspect_tolerance=0.05):
        """
        Check of het logo niet gestretched of vervormd is.
        Vergelijkt uitsluitend de aspect ratio (breedte/hoogte), niet de
        absolute pixelafmetingen — een geüploade flyer heeft vrijwel nooit
        exact dezelfde resolutie als het referentiebestand, dat is geen
        vervorming.
        """
        ...
        if h == 0 or ref_h == 0:
            return False, "Ongeldige afbeeldingsafmetingen"

        aspect_ratio = w / h
        ref_aspect = ref_w / ref_h

        # Relatief verschil t.o.v. de referentie-aspect ratio
        relative_diff = abs(aspect_ratio - ref_aspect) / ref_aspect

        if relative_diff > aspect_tolerance:
            return False, (
                f"Aspect ratio afwijkend: {aspect_ratio:.2f} vs {ref_aspect:.2f} "
                f"({relative_diff * 100:.1f}% verschil)"
            )

        return True, f"Geen vervorming gedetecteerd (aspect ratio: {aspect_ratio:.2f})"
    ...
    def match_logo(self, uploaded_img_array):
        ...
        for name, ref_array in self.reference_logos.items():
            try:
                # Resize de UPLOAD naar het referentieformaat (niet andersom),
                # zodat de vaste referentie-aspect ratio behouden blijft en
                # SSIM niet vertekend raakt door een omgekeerde resize.
                ref_h, ref_w = ref_array.shape[:2]
                if uploaded_img_array.shape[:2] != (ref_h, ref_w):
                    upload_pil = Image.fromarray(uploaded_img_array)
                    upload_resized = upload_pil.resize((ref_w, ref_h))
                    upload_array_resized = np.array(upload_resized)
                else:
                    upload_array_resized = uploaded_img_array

                # Bereken gelijkenis (SSIM) - alleen RGB
                if upload_array_resized.shape[2] >= 3 and ref_array.shape[2] >= 3:
                    upload_rgb = upload_array_resized[:, :, :3]
                    ref_rgb = ref_array[:, :, :3]

                    # Resize voor SSIM (als te groot), beide met dezelfde factor
                    ...
                    try:
                        score = ssim(upload_rgb, ref_rgb, channel_axis=2, data_range=255)
                    except Exception:
                        # Fallback: gebruik mean squared error
                        mse = np.mean((upload_rgb.astype(float) - ref_rgb.astype(float)) ** 2)
                        score = 1 - (mse / (255 ** 2))
    ...
    def check_english_logo(self, img_array):
        ...
        for name, eng_array in self.english_logos.items():
            try:
                eng_h, eng_w = eng_array.shape[:2]
                if img_array.shape[2] >= 3 and eng_array.shape[2] >= 3:
                    # Resize de UPLOAD naar het formaat van het engelse referentielogo
                    if img_array.shape[:2] != (eng_h, eng_w):
                        upload_pil = Image.fromarray(img_array)
                        upload_resized = upload_pil.resize((eng_w, eng_h))
                        upload_rgb = np.array(upload_resized)[:, :, :3]
                    else:
                        upload_rgb = img_array[:, :, :3]

                    eng_rgb = eng_array[:, :, :3]

                    score = ssim(upload_rgb, eng_rgb, channel_axis=2, data_range=255)
...
def main():
    ...
    * ❌ Of er geen Engelstalige logo's zijn gebruikt
    ...
    with st.sidebar:
        ...
        - Engelstalige logo's moeten eindigen op '_en' (bv. `logo_en.png`)
        """)
